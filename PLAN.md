...
## 将来の外部DB移行を見据えた実装メモ（開発中プレビュー優先）

> 現在は UI/体験を固めるフェーズのため、直ちに外部DB（Supabase等）へ全面移行はしない。
> ただし、後戻りコストを抑えるため「外部DBを前提にした境界設計」を先に進める。

### 方針

- [ ] 当面は `localStorage` 運用を継続し、画面仕様の検証速度を優先する。
- [ ] データアクセスを `storage/appStorage.js` に閉じ込め、UI層から永続化実装詳細を参照しない。
- [ ] 将来の切替先（Supabase等）を想定し、`load/save` の呼び出し箇所を増やしすぎない。

### 実装ガードレール（今から守る）

- [ ] IDはクライアント生成でも UUID を継続し、DB主キー移行時の整合を取りやすくする。
- [ ] 画面stateと保存payloadのスキーマ差分を意識し、保存前の整形関数を追加できる形にする。
- [ ] 削除・更新処理はハンドラに散らさず、将来API化しやすい単位（Player/Company/Revenue）で関数化する。
- [ ] 「ラウンド確定」など重要操作は、将来の監査ログ追加を想定して操作ポイントを明確化する。

### DB移行の着手トリガー

- [ ] 複数端末で同時利用の検証を開始するタイミング。
- [ ] ルール検証が一巡し、UI文言・入力項目の大きな変更が減ったタイミング。
- [ ] 参加者ごとの閲覧/編集権限を付ける必要が発生したタイミング。

### 移行時の最小スコープ（再作り直し回避）

- [ ] `gameId` を導入（URLまたは状態に保持）。
- [ ] `players/companies` から先行して外部DB化。
- [ ] 次段で `stockHoldings/orRevenues` を移行し、最後にリアルタイム同期を追加。
...
- [x] `TrainCard` 内の「削除」「クリア」等を共通 `Button` へ寄せる
- [x] プレイヤー/企業削除アイコンボタンの variant 整理（Danger統一）
- [x] OR入力周辺のクイック操作ボタン群の共通化方針を決める
...
- [x] 見出し・本文・補助テキストのサイズ利用ルールを `PLAN.md` に明文化
- [x] 主要入力 UI（収益編集 / 割合入力 / モーダル）へ統一ルール適用
- [x] 320px 幅での可読性チェック（テキスト折返し、ボタン高さ、タップ領域）

**タイポグラフィ運用ルール（4-7B）**

- ページ見出しは `SectionHeader size="page"`（`text-3xl`）を使用し、セクション見出しは `size="section"`（`text-2xl`）で統一する。
- 本文・説明文は `text-ui-base` を基本とし、補助文（注釈/状態ラベル）は `text-ui-sm` または `text-ui-xs` を使用する。
- 主要入力操作（収益編集、割合入力、モーダル内ボタン）は `Input` / `Button` を経由し、タップ領域を `py-1.5` 以上で確保する。
- 画面幅 320px を最小想定として、折返しが発生する文言は2行以内を目安に調整する。
- 色だけで意味を伝える箇所（企業切替ピル等）は、状態ラベル（例: 選択中/収益あり/未入力）を併記し、色覚特性に依存しない判別を確保する。
...
- [x] 破壊操作（削除/リセット）の Danger 統一を全画面で完了
- [x] 通常編集操作を Primary/Neutral に寄せる
- [x] 「色だけで意味を伝えるUI」の棚卸しとラベル併記方針の記録
...
- [x] 現在の `App.jsx` セクションを「設定 / 入力 / 確認」に分類
- [x] セクション並び順をユーザー操作順へ再配置
- [x] 破壊操作の下部集約
- [x] `src/views/management` / `src/views/company-detail` / `src/views/summary` へ分割
...
※ 本スプリントでは 4-7E を実装スコープ外とし、4-7D 完了後に再評価する。
