...
- [x] **Phase 3-5** ESLint/Prettier と pre-commit 整備
- [x] **Phase 3-6** CRA → Vite 移行（Pages設定・CI更新を含む）
- [ ] **Phase 4-7** UI/デザイン見直し計画 1〜5 の残タスク完了
  - 進捗: UIトークン（Tailwind）拡張、共通UIコンポーネント（Button/Card）追加、初回導線/タブ進捗表示を実装。
  - 進捗: 共通 Input コンポーネントを追加し、主要入力（収益値編集/カスタム収益/割合入力/名称編集）へ段階適用。モーダル操作ボタンも共通 Button へ置換。
  - 進捗: `Modal` の UI コンポーネント分離と `SectionHeader` 導入を実施し、4-7A（デザインシステム整備）の完了条件を満たす状態まで整理。
  - 進捗: 収益編集/割合入力まわりのクイック操作を `Button` 経由に統一し、`text-ui-*` ベースのタイポグラフィ運用を `PLAN.md` に明文化。
  - 進捗: 管理/企業詳細ビューの主要アクションを `Button` コンポーネントへ置換し、削除・リセットの `danger` 統一と通常操作の Primary/Secondary 寄せを実施。
  - 進捗: 320px幅での表示確認を実施し、企業切替ピルに状態ラベル（選択中/収益あり/未入力）を追加して「色のみ依存」の判別を解消。
  - 進捗: CompanyDetailView の情報区分を「設定 / 入力 / 確認」で見出し分類し、4-7D の第一段階（分類）に着手。4-7E は本スプリントの実装スコープ外として管理。
  - 進捗: 4-7D を完了。`SummaryView` / `ManagementView` / `CompanyDetailView` を `src/views/*` 配下へ分割し、全般管理の破壊操作を下部セクションへ集約して操作順を明確化。
  - メモ: サマリー未完了ガイド/状態ラベルは優先度を下げ、後続のIA再設計と合わせて再検討する。
...
- Phase 3-6 実施: npm scripts を Vite/Vitest ベースへ移行し、`vite.config.js` で Pages 用 `base` を設定。CI は Node 22 + `dist` アーティファクトへ更新。
